function ducktape::haproxy_http_edge(
  ...
  $http_edge_redirect_rules = flatten($http_edge_domains_envs.map |$domain, $options| {
      ...
      $options['_envs'].reduce([]) |$memo, $env| {
        ...
        # Extra rule to add support for query strings. It is a regm match
        # on the url. It is applied for files named after "$domain.url_regm.map".
        # #TODO# This is generalizable to ${fetch}_${matchtype}, being "path"
        # the default fetch (for backwards compatibility) if no underscore
        # is present.
        $type = 'regm'
